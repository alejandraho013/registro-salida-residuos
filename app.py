...
# --- FUNCIÓN PARA GENERAR EL EXCEL CON LAS DOS PESTAÑAS ---
def generar_excel_optimo(df_detalle, df_resumen):
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df_detalle.to_excel(writer, sheet_name='Detalle Filtrado', index=False)
        df_resumen.to_excel(writer, sheet_name='Resumen por Empresa', index=False)
...
# ─────────────────────────────────────────────────────────────
# TAB 2: REPORTES (AJUSTADO PARA DESCARGA EXCEL)
# ─────────────────────────────────────────────────────────────
with tab2:
    ...
    if df_master.empty:
        ...
        # ── BOTÓN DE DESCARGA EN EXCEL (AJUSTADO) ──
        st.markdown("---")
        excel_descarga = generar_excel_optimo(df_tabla, resumen)
        st.download_button(
            label="⬇️ Descargar Reporte en EXCEL (Detalle + Resumen)",
            data=excel_descarga,
            file_name=f"Reporte_TINTATEX_{datetime.now().strftime('%Y%m%d')}.xlsx",
